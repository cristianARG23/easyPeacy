...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write('################################################################################################################################################## \n\n')
		res.write('Total de peliculas en cartelera: ' + JSON.stringify(movies.length))
		res.write('\n\n\n')
		...
		case '/mas-votadas':
			...
			let rating7 = movies.filter(function(rating){
				return rating.vote_average >= 7;
			})
			let ratingMap = rating7.map(function (rank){
				return rank.vote_average
			})
			let sumaRank = ratingMap.reduce(function (acum,num){
				return (acum + num);
			})
			let promedioRank = sumaRank / rating7.length;
			let promedioRedondeado = promedioRank.toFixed(1)
			res.write('El promedio de todas las peliculas es: ' + promedioRedondeado);

			movies.sort((a,b)=>(a.title > b.title) ? 1 : (a.title < b.title) ? -1 : 0);
				rating7.forEach(movies => {
					res.write('\n\n')
					res.write('-' + movies.title + ': ')
					res.write('\n\n')
					res.write(movies.overview)
					res.write('\n\n')
					res.write('El rating es: ' + movies.vote_average)
				})
			res.end();
			break;
		case '/sucursales':
			res.write('Sucursales \n\n')
			res.write('Total de salas: ' + JSON.stringify(theaters.length))
			res.write('\n\n')
			theaters.forEach(theaters =>{
				res.write('\n\n')
				res.write(theaters.name);
				res.write('\n\n')
				res.write(theaters.address)
				res.write('\n\n')
				res.write(theaters.description)
			})
			res.end();
			break;
		case '/contacto':
			...
			res.write('Preguntas Frecuentes')
			res.write('\n\n')
			res.write('Consultas totales: ' + JSON.stringify(faqs.length))
			faqs.forEach(faqs =>{
				res.write('\n\n')
				res.write('Consulta: ' + faqs.faq_title)
				res.write('\n\n')
